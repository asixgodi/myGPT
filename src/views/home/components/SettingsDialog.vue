...
<template>
  <!-- 这里写update:model-value是为了，用户点击遮罩层外面时，关闭对话框 -->
  <el-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" :width="width" center>
    <div :class="{ 'p-6': !isMobile, 'p-3': isMobile }">
      ...
      <div :class="['flex items-center gap-4', { 'flex-col items-start': isMobile }]">
        ...
        <el-input v-model="localKey" placeholder="sk-xxxxxxxxxx" :size="isMobile ? 'default' : 'large'" clearable class="flex-1" :class="{ 'w-full': isMobile }" />
      </div>
      <div :class="['flex items-center gap-4 mt-5', { 'flex-col items-start': isMobile }]">
        <span class="font-bold whitespace-nowrap">Version</span>
        <el-select v-model="localVersion" :size="isMobile ? 'default' : 'large'" class="flex-1" :class="{ 'w-full': isMobile }">
          <el-option v-for="item in gptVersionList" :key="item.id" :label="item.id" :value="item.id" />
        </el-select>
      </div>
    </div>
    <template #footer>
      <div :class="[{ 'flex justify-center items-center gap-4': !isMobile, 'space-y-2': isMobile }]">
        <el-button @click="$emit('update:modelValue', false)" :size="isMobile ? 'default' : 'large'" :class="{ 'w-full mb-2': isMobile }"> Cancel </el-button>
        <el-button type="primary" @click="emit('save', localKey, localVersion)" :size="isMobile ? 'default' : 'large'" class="!m-0" :class="{ 'w-full': isMobile }"> Save </el-button>
      </div>
    </template>
  </el-dialog>
</template>
